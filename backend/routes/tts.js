...
function processTextToSegments(text, defaultLang = 'en') {
  ...
  sentences.forEach((sentence) => {
    ...
    let currentLang = hanziRegex.test(sentence) ? 'zh' : defaultLang;
...
function generateSSML(text, langAttr, voiceFromFrontend) {
  console.log("🎤 generateSSML received voice:", voiceFromFrontend);
  console.log("🎤 generateSSML received langAttr:", langAttr);
  
  const langKey = langAttr.split('-')[0];
  const segments = processTextToSegments(text, langKey);
  
  // Determine voice based on frontend input or fallback
  let primaryVoice = voiceFromFrontend;
  if (!primaryVoice) {
    const langKey = langAttr.split('-')[0];
    primaryVoice = customVoices[langKey] || 'en-US-JennyNeural';
  }
  
  console.log("🔊 Using primary voice:", primaryVoice);

  const ssmlParts = segments.map(seg => {
    // Use segment-specific voice or primary voice
    const segmentVoice = seg.lang === 'zh'
    ? customVoices.zh
    : (voiceFromFrontend || customVoices[seg.lang] || primaryVoice);
    const rate = seg.lang === 'zh' ? '0.9' : '1.0';
    const escapedContent = escapeSSML(seg.content);
    console.log("🎯 Segment:", seg.lang, "Voice:", segmentVoice);
    return `<voice name="${segmentVoice}"><prosody rate="${rate}">${escapedContent}</prosody></voice>`;
  });
  
  const ssml = `<speak version="1.0" xmlns="http://www.w3.org/2001/10/synthesis" xml:lang="${langAttr}">${ssmlParts.join(' ')}</speak>`;
  console.log("📦 Final SSML:", ssml);
  return ssml;
}
...
router.post('/speak', async (req, res) => {
  ...
  console.log("🚀 Voice received from frontend:", voice);

  let langAttr = voice?.split('-').slice(0, 2).join('-') || 'en-US';
  ...
  try {
    ...
    if (voice) {
      ...
      console.log("🔊 Voice akan ditentukan oleh SSML, bukan manual:", voice);
    }
    ...
    const ssml = generateSSML(cleanedText, langAttr, voice);
    ...
    const timeout = setTimeout(() => {
      if (!isResponded) {
        console.error("⏱ TTS operation timed out after 12s");
        synthesizer.close();
        return res.status(504).json({ error: "TTS timeout after 12 seconds" });
      }
    }, 12000);
    console.log("🎤 Calling speakSsmlAsync with voice:", voice);
    ...
    console.error("🔥 INTERNAL ERROR in /speak:", err);
