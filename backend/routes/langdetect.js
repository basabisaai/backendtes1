...
function detectLanguagesWithScore(text) {
  try {
    ...
    return detections.map(d => ({
      lang: d.lang.startsWith('zh') ? 'zh-CN' : (d.lang === 'id' ? 'id' : d.lang),
      prob: d.prob
    }));
  } catch (err) {
...
const forbiddenLangs = ['ms', 'jv', 'su', 'tl', 'nl', 'fr', 'de', 'vi', 'hi', 'ko', 'ja', 'ru', 'ar', 'es', 'pt'];
...
router.post('/langdetect', async (req, res) => {
  ...
  try {
    ...
    const allowedLangs = ['en', 'zh-CN', 'id'];
    ...
    if (onlyAllowedLanguages && !containsForbidden) {
      if (langsSet.size === 1) {
        if (langsSet.has('en')) {
          ...
          language = 'zh-CN';
          isSupported = true;
        } else if (langsSet.has('id')) {
          language = 'id-ID';
          isSupported = true;
        }
      } else if (langsSet.size === 2) {
        if (langsSet.has('en') && langsSet.has('zh-CN')) {
          language = 'mix-en-zh';
          isSupported = true;
        } else if (langsSet.has('id') && langsSet.has('zh-CN')) {
          language = 'mix-id-zh';
          isSupported = true;
        }
      }
...
function detectLanguage(text) {
  const detections = detectLanguagesWithScore(text);
  return detections.length > 0 ? detections[0].lang : 'unknown';
}

function detectLanguages(text) {
  const langs = new Set();
  const parts = text.split(/[.!?。\n]+/); // pisah per kalimat atau titik

  for (const part of parts) {
    const clean = part.trim();
    if (!clean) continue;

    const lang = detectLanguage(clean); // Pake fungsi lu sendiri yg udah bagus
    langs.add(lang);
  }
...
router.detectLanguage = detectLanguage;
router.detectLanguages = detectLanguages;
